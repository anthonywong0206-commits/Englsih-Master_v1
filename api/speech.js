...
function guessDifficultWords(text = '') {
  const commonHard = ['restaurant', 'reservation', 'available', 'appointment', 'recommend', 'comfortable', 'vegetarian', 'colleague', 'schedule', 'interview']
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      note: aiScore.note || '已完成語音轉文字及 AI 口語分析。'
    })
  } catch (error) {
    console.error(error)
